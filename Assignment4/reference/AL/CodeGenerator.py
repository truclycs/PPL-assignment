...
class CodeGenVisitor(BaseVisitor, Utils):
    ...
    def visitProgram(self, ast, c):
        ...
        #env1  = functools.reduce(lambda y,x: y + ([Symbol(x.variable.name,x.varType, CName(self.className))] if type(x) is VarDecl else [Symbol(x.name.name, MType([vt.varType for vt in x.param],x.returnType), CName(self.className))]), ast.decl,self.env if self.env else [])

        lsFun       = list(filter(lambda x:type(x) is FuncDecl, ast.decl))
        functools.reduce(lambda a,b: self.visit(b,a), lsFun, SubBody(None,nenv))
    ...
    def genMETHODINIT(self,consdecl,o,frame):
        #consdecl   : FuncDecl
        #o          : Any 
